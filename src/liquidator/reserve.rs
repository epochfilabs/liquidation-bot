//! Reserve account deserialization from raw bytes.
//!
//! Extracts the fields needed for flash loan liquidation from the on-chain
//! Reserve account data using known byte offsets from the klend source.
...
const RESERVE_MIN_SIZE: usize = 8 + 5784; // 8-byte disc + Reserve struct
...
mod offsets {
    ...
    pub const LIQUIDITY_MINT: usize = 128;        // Pubkey (32 bytes)
    pub const LIQUIDITY_SUPPLY_VAULT: usize = 160; // Pubkey (32 bytes)
    pub const LIQUIDITY_FEE_VAULT: usize = 192;   // Pubkey (32 bytes)
    pub const LIQUIDITY_AVAILABLE: usize = 224;    // u64 (8 bytes)
    pub const LIQUIDITY_BORROWED_SF: usize = 232;  // u128 (16 bytes)
    pub const LIQUIDITY_MARKET_PRICE_SF: usize = 248; // u128 (16 bytes)
    pub const LIQUIDITY_TOKEN_PROGRAM: usize = 408; // Pubkey (32 bytes)

    // ReserveCollateral fields (struct starts at byte 2552, + 8 disc = 2560)
    pub const COLLATERAL_MINT: usize = 2560;           // Pubkey (32 bytes)
    pub const COLLATERAL_SUPPLY_VAULT: usize = 2600;   // Pubkey (32 bytes)
    ...
    pub const FLASH_LOAN_FEE_SF: usize = 4904; // u64 (8 bytes) — origination_fee is at 4896
}
...
pub fn parse_reserve(reserve_pubkey: &Pubkey, data: &[u8]) -> Result<ReserveData> {
    if data.len() < RESERVE_MIN_SIZE {
        bail!(
            "reserve account too small: {} bytes (expected >= {})",
            data.len(),
            RESERVE_MIN_SIZE
        );
    }

    let liquidity_mint = read_pubkey(data, offsets::LIQUIDITY_MINT);
    let liquidity_supply_vault = read_pubkey(data, offsets::LIQUIDITY_SUPPLY_VAULT);
    let liquidity_fee_vault = read_pubkey(data, offsets::LIQUIDITY_FEE_VAULT);
    let liquidity_token_program = read_pubkey(data, offsets::LIQUIDITY_TOKEN_PROGRAM);
    let collateral_mint = read_pubkey(data, offsets::COLLATERAL_MINT);
    let collateral_supply_vault = read_pubkey(data, offsets::COLLATERAL_SUPPLY_VAULT);
    ...
    Ok(ReserveData {
        accounts: ReserveAccounts {
            ...
            token_program: liquidity_token_program,
        },
...
/// Parse the LendingMarket account to get liquidation parameters.
#[derive(Debug, Clone)]
...
/// Offsets within LendingMarket account data (includes 8-byte Anchor discriminator).
mod market_offsets {
    pub const LIQUIDATION_MAX_DEBT_CLOSE_FACTOR_PCT: usize = 126; // u8
    pub const MAX_LIQUIDATABLE_DEBT_MARKET_VALUE_AT_ONCE: usize = 136; // u64
}
